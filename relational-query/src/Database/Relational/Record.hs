...
unsafeFromSqlTerms :: [StringSQL] -> Record i j c t
unsafeFromSqlTerms = Syntax.typeFromRawColumns


-- | Unsafely trace projection path.
unsafeProject :: PersistableRecordWidth a -> Record i j c a' -> Pi a b -> Record i j c b'
...
wpi :: PersistableRecordWidth a
    ...
    -> Record i j c b -- ^ Narrower 'Record'
wpi =  unsafeProject
...
piMaybe :: PersistableWidth a
        ...
        -> Pi a b             -- ^ Projection path
        -> Record i j c (Maybe b) -- ^ Narrower 'Record'. 'Maybe' type result
...
unsafeToFlat :: Record i j Aggregated r -> Record i j Flat r
unsafeToFlat =  unsafeChangeContext

notNullMaybeConstraint :: HasColumnConstraint NotNull r => Record i j c (Maybe r) -> NotNullColumnConstraint r
notNullMaybeConstraint =  const KeyConstraint.columnConstraint

-- | Unsafely get SQL string expression of not null key record.
unsafeStringSqlNotNullMaybe :: HasColumnConstraint NotNull r => Record i j c (Maybe r) -> StringSQL
