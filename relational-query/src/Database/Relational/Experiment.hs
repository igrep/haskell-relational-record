...
-- igrep NOTE: Delete record label based on the result of restrict?
...
class (IxFunctor m, IxMonad m) => MonadPlaceHolders m xs ys where
  ph
    :: forall key v
     . (Show v, HWithout (key >: v) xs ys, Associate key v xs)
    => FieldName key
    -> m (Record xs) (Record ys) ()
