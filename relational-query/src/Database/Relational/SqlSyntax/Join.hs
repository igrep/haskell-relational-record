...
growRight :: Maybe (Node (DList Tuple))            -- ^ Current tree
          -> (NodeAttr, ProductTree (DList Tuple)) -- ^ New tree to push into right
          -> Node (DList Tuple)                    -- ^ Result node
growRight = d  where
  d Nothing  (naR, q) = Node naR q
  d (Just l) (naR, q) = Node Just' $ Join l (Node naR q) mempty

-- | Push new leaf node into product right term.
growProduct :: Maybe (Node (DList Tuple))      -- ^ Current tree
...
restrictProduct' :: ProductTree (DList Tuple) -- ^ Product to restrict
                 -> Tuple                     -- ^ Restriction to add
                 -> ProductTree (DList Tuple) -- ^ Result product
...
restrictProduct :: Node (DList Tuple) -- ^ Target node which has product to restrict
                -> Tuple              -- ^ Restriction to add
                -> Node (DList Tuple) -- ^ Result node
