...
value :: (LiteralSQL t, OperatorContext c) => t -> Record i j c t
...
values :: (LiteralSQL t, OperatorContext c) => [t] -> RecordList i j (Record i j c) t
...
-- So the type should be Record (xs -> ys) c t
...
exists :: OperatorContext c
       => RecordList i j (Record i j Exists) r -> Record i j c (Maybe Bool)
...
in' :: OperatorContext c
    => Record i j c t -> RecordList i j (Record i j c) t -> Record i j c (Maybe Bool)
